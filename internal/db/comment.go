...
func (d *Database) GetComment(ctx context.Context, uuid string) (comment.Comment, error) {
	...
	row := d.Client.QueryRowxContext(ctx,
		`SELECT id,slug,body,author FROM comments WHERE text(id)=$1`, uuid)
...
func (d *Database) DeleteComment(ctx context.Context, id string) error {
	_, err := d.Client.ExecContext(ctx,
		`DELETE FROM comments WHERE text(id)=$1`,
		id,
	)
	if err != nil {
		return fmt.Errorf("failed to delete a comment from database: %w", err)
	}
	return nil
}
...
func (d *Database) UpdateComment(ctx context.Context, id string, cmt comment.Comment) (comment.Comment, error) {
	...
	rows, err := d.Client.NamedQueryContext(ctx,
		`UPDATE comments SET slug=:slug,author=:author,body=:body WHERE text(id)=:id`,
